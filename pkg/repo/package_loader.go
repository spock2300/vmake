...
`, l.vmakeDir)
	...
	downloadCmd.Dir = buildDir
	downloadCmd.Env = append(os.Environ(),
		"GO111MODULE=on",
		"GOPROXY=https://goproxy.cn,direct",
	)
	if output, err := downloadCmd.CombinedOutput(); err != nil {
		return "", fmt.Errorf("go mod tidy error: %s", string(output))
	}

	pluginPath := filepath.Join(buildDir, "plugin.so")
	cmd := exec.Command("go", "build", "-buildmode=plugin", "-o", "plugin.so", "package.go")
	cmd.Dir = buildDir

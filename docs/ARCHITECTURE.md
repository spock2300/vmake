...
| Go 编译 | `internal/gocompile/` | Go 插件编译工具（仅扩展系统使用） |
...
### 插件加载流程（扩展系统，仍使用 .so 编译）

> 注意：buildscript 系统已改用 yaegi 解释器，不再编译 .so。以下流程仅适用于扩展插件系统（`vmake ext`）。
...
`plugin.CompileResult` 嵌入此基础结构（仅扩展系统使用，buildscript 不再使用 gocompile）：

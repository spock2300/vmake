...
- 构建脚本就是标准 Go 代码，在编写时有 Go 工具链的类型检查，在运行时由解释器执行（无编译期强制，脚本错误在加载时暴露）
...
- 增量编译——比较动作签名与源文件、depfile 依赖和产物的内容指纹；目标串行，目标内部按 `-j` 并行
...
VMake 不是对 CMake 的简单替代，而是一次**重新思考 C/C++ 工程管理应该是什么样子**的尝试。它用 Go 的类型体系（编辑期检查 + 运行时解释加载即报错）取代了 DSL 的脆弱，用 Registry + Native 双源包管理消除了代码分享的障碍，用三阶段执行模型理清了构建的逻辑。
